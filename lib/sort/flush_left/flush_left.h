...
#ifndef __FLUSH_LEFT_H__
#define __FLUSH_LEFT_H__

#include <stdint.h>

/**
 * @brief   左移，主要用于字符数组的左移，移动指定的数组，把前面的标志性字符去除，
 * 				其他的左移，在后面在添加上标志性字符。
...
/**
 * @brief   左移，主要用于字符串的左移。移动指定字符数组，把前面的标志性字符去除，
 * 				其他的左移，在后面加上'\0'结束。
 ...
 * @param   lFlag 前面需要清除的标志性字符
 */
void FlushLeftStr(uint8_t *lData, uint8_t lLength, uint8_t lFlag);
